<template>
	<div class="container" @dragover.prevent="dragOver" @dragleave.prevent="dragLeave" @drop.prevent="drop($event)">
		...
		<div v-show="Imgs.length === 0 && imgProp.length === 0" class="beforeUpload">
			<input ref="uploadInput" type="file" style="z-index: 1" accept="image/*" multiple @change="previewImgs" />
		...
		<div v-show="Imgs.length > 0 || imgProp.length > 0" class="imgsPreview">
			<button v-if="max !== 1" type="button" class="clearButton" @click="reset">
				Удалить все
			</button>
			<div v-for="(img, i) in Imgs.length > 0 ? Imgs : imgProp" :key="i" class="imageHolder">
				<img :src="img" />
				...
				<div v-if="max !== 1" class="plus" @click="append">+</div>
			</div>
		</div>
	</div>
</template>
